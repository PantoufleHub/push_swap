...
/*   Updated: 2024/02/10 20:18:19 by aperron          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sort(t_push_swap *push_swap)
{
	if (is_finished(push_swap))
		return ;
	else if (nb_in_stack(push_swap->a) == 2)
		sa(push_swap);
...
{
	return (value_index(stack, index_biggest(stack)));
}

int	smallest_value(t_stack *stack)
{
	return (value_index(stack, index_smallest(stack)));
}
